...
public class DarknessEnemy : MonoBehaviour, IEnemyInterface
{
    #region Variables
    ...
    IEnumerator Ability()
    {
        ...
        while (this.gameObject.activeSelf)
        {
            ...
            target = Physics2D.OverlapCircle(transform.position, ES.abilityRange, GameManager.Instance.CollectableLayer);

            if (target)
            {
                target.gameObject.SetActive(false);

                buffAmount += ES.abilityValue;
